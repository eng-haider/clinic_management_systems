  ...
  <div>
    <v-dialog v-model="casesheet" max-width="1200px" v-track-dialog>
      <cases
        ...
        :CaseCategories="CaseCategoriess"
      ></cases>
    </v-dialog>
    ...
    <v-container id="dashboard" fluid tag="section">
      ...
      >
        ...
        <template v-slot:item.tooth_num="{ item }">
          <span
            v-for="(num, index) in parseToArray(item.tooth_num)"
            :key="index"
            style="font-weight: bold"
          >
            {{ num }}
            <span v-if="index < parseToArray(item.tooth_num).length - 1">
              -
            </span>
          </span>
        </template>
        ...
        <template v-slot:[`item.doctor`]="{ item }">
          <div v-if="item.doctors.length > 0">
            ...
            <v-chip
              style="margin: 2px"
              color="primary"
              v-for="item in item.doctors"
              :key="item"
            >
              <v-icon left> mdi-account-circle-outline </v-icon>{{ item.name }}
            </v-chip>
          </div>
        </template>

        <template v-slot:[`item.phones`]="{ item }">
          <p style="direction: ltr; text-align: end">{{ item.phone }}</p>
        </template>
        ...
        <template v-slot:[`item.cases`]="{ item }">
          ...
          <v-btn
            v-else
            dense
            @click="$router.push('/admin/case/' + item.id)"
            color="#0a304ed4"
        ...
        <template v-slot:[`item.status`]="{ item }">
          <v-switch
            ...
            :label="
              item.status.id === 43 ? $t('completed') : $t('not_completed')
            "
...
export default {
  ...
  methods: {
      ...
      this.axios
        ...
        .then((response) => {
          ...
          Object.keys(localStorage).forEach((key) => {
            if (
              key.includes("cases_cache") ||
              key.includes("search_cases_cache")
            ) {
              localStorage.removeItem(key);
            }
          });
    ...
    editItem(item) {
      ...
      if (this.editedItem.images.length > 0) {
        this.imageSource =
          "https://mina-api.tctate.com/public/images/" +
          this.editedItem.images[0].image_url;
      }

      // Check if `root_stuffing` is a string and attempt to parse it
      if (typeof this.editedItem.root_stuffing === "string") {
        try {
          this.editedItem.root_stuffing = JSON.parse(
      ...
      if (
        !this.editedItem.root_stuffing ||
        typeof this.editedItem.root_stuffing !== "object"
      ) {
        this.editedItem.root_stuffing = {};
      }
    ...
    getByDocor() {
      ...
      axios
        .get("cases/getByDoctor/" + this.searchDocorId, {
          headers: {
            "Content-Type": "application/json",
            Accept: "application/json",
            Authorization: "Bearer " + this.$store.state.AdminInfo.token,
    ...
    seachs() {
      ...
      if (
        this.search.case_categores_id == null ||
        this.search.case_categores_id === undefined
      ) {
      ...
      if (
        ...
        this.search.status_id === undefined
      ) {
      ...
      let searchUrl =
        "cases/searchv3?filter[status_id]=" +
        this.search.status_id +
        "&filter[case_categores_id]=" +
        this.search.case_categores_id +
        "&filter[sessions.note]=" +
        this.search.note +
        "&filter[is_paid]=" +
        this.search.is_paid;
    ...
    getCaseCategories() {
      ...
      console.log(
        "� Token:",
        this.$store.state.AdminInfo?.token ? "Token exists" : "No token",
      );
    ...
    initialize() {
      ...
      axios
        .get(`cases/UserCasesv2?page=${this.current_page}`, {
          headers: {
            "Content-Type": "application/json",
            Accept: "application/json",
            Authorization: "Bearer " + this.$store.state.AdminInfo.token,
        ...
        if (this.editedIndex > -1) {
          ...
          this.axios
            ...
            .then((res) => {
              ...
              (this.dialog = false), this.initialize();
