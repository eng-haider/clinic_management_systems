<template>
  <div>
                ...
                <v-container>
                  <v-row>
                    <v-col class="py-0" cols="12" sm="6" md="6">
                      <v-text-field
                        v-model="editedItemCategories.name"
                        style="direction: rtl; text-align: right"
                        :rules="[rules.required]"
                        :label="$t('store.nameCate')"
                        outlined
                      >
                      </v-text-field>
                    </v-col>
    ...
    <v-container id="dashboard" fluid tag="section">
      <v-expansion-panels>
        <v-expansion-panel
          v-for="item in ConjugationsCategories"
          :key="item.id"
            ...
            <v-layout row wrap>
              ...
              <v-flex xs12 md3 sm3 pt-3>
                <span v-if="item.conjugations.length > 0">
                  {{ item.conjugations[0].total_price | currency }} د.ع
                </span>
              </v-flex>
              ...
              <v-flex xs12 md2 sm2 pt-3>
                <div v-if="item.conjugations.length > 0">
                  ...
                  {{ item.conjugations[0].paid | currency }} د.ع
                </div>
              </v-flex>
              ...
              <v-flex xs12 md3 sm3 pt-3>
                <div v-if="item.conjugations.length > 0">
                  ...
                  {{
                    (item.conjugations[0].total_price -
                      item.conjugations[0].paid)
                      | currency
                  }}
                  د.ع
                </div>
          ...
          <v-expansion-panel-content>
            <div v-if="selectedCategoryData[item.id]">
              <v-data-table
                ...
                :items="selectedCategoryData[item.id]"
                class="elevation-1"
              >
                ...
                <v-row>
                  ...
                  <v-col cols="12" md="6" sm="6" class="display:none">
                    <v-select
                      :rules="[rules.required]"
                      dense
                      v-model="editedItem.conjugations_categories_id"
                      required
                      label=" النوع "
                      :items="ConjugationsCategories"
                      outlined
                      item-text="name"
                      item-value="id"
                    >
                  ...
                  <v-col cols="12" md="6" sm="6">
                    <v-switch
                      ...
                      :label="editedItem.is_paid ? $t('paid') : $t('not_paid')"
                      color="green"
                      inset
                    ></v-switch>
                  </v-col>
...
export default {
  ...
  methods: {
    ...
    editItemItem(item) {
      this.editedIndex =
        this.selectedCategoryData[item.conjugations_categories_id].indexOf(
          item,
        );
      ...
      Swal.fire({
        ...
        if (result.value) {
          Axios.delete("ConjugationsCategories/" + item.id, {
            ...
            .catch(() => {
              this.$swal.fire(
                "لا يمكن حذف قسم يحتوي علئ فواتير",
                this.$t("not_done"),
                "error",
              );
            });
    ...
    saveCat() {
      if (this.$refs.form.validate()) {
        ...
        if (this.editedCategoryIndex > -1) {
          this.axios
            .patch(
              "https://mina-api.tctate.com/api/ConjugationsCategories/" +
                this.editedItemCategories.id,
              this.editedItemCategories,
              {
                headers: {
                  "Content-Type": "application/json",
                  Accept: "application/json",
    ...
    save() {
      if (this.$refs.form.validate()) {
        ...
        if (this.editedIndex > -1) {
          this.axios
            ...
            .then(() => {
              ...
              this.fetchDataByCategory(
                this.editedItem.conjugations_categories_id,
              );
              this.close();
              this.initialize();
              this.$swal.fire({
          ...
          this.axios
            .post(
              "https://mina-api.tctate.com/api/Conjugations",
              this.editedItem,
              {
                headers: {
                  "Content-Type": "application/json",
            ...
            .then((res) => {
              ...
              (this.dialog = false), (this.loadSave = false);
              // this.initialize();
              this.fetchDataByCategory(
                this.editedItem.conjugations_categories_id,
              );
    ...
    getConjugationsCategories() {
      this.loading = true;
      Axios.get(
        "https://mina-api.tctate.com/api/Conjugations/ConjugationsCategories",
        {
          headers: {
            "Content-Type": "application/json",
            Accept: "application/json",
    ...
    fetchDataByCategory(categoryId) {
      Axios.get(
        `https://mina-api.tctate.com/api/Conjugations/getByCat/${categoryId}`,
        {
          headers: {
            "Content-Type": "application/json",
            Accept: "application/json",
    ...
    initialize() {
      this.loading = true;
      Axios.get("https://mina-api.tctate.com/api/ConjugationsCategories", {
        headers: {
          "Content-Type": "application/json",
          Accept: "application/json",
          Authorization: "Bearer " + this.$store.state.AdminInfo.token,
    ...
    updateIsPaid(item) {
      Axios.patch(
        `https://mina-api.tctate.com/api/Conjugations/updateIsPaid/${item.id}`,
        {
