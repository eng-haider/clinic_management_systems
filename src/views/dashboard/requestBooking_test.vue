<template>
  <v-container fluid>
    ...
    <v-app-bar
      v-if="
        ($store.state.role === 'secretary' ||
          $store.state.role === 'adminDoctor' ||
          $store.state.role === 'accounter') &&
        doctors &&
        doctors.length > 1
      "
      color="primary"
      ...
      <v-card>
              ...
              <v-col cols="12" md="6">
                <v-list dense>
                  ...
                  <v-list-item>
                    <v-list-item-content>
                      <v-list-item-title>
                        <strong>{{ $t("datatable.doctor") }}:</strong>
                        {{
                          book_details.owner_name || book_details.doctor_name
                        }}
                      </v-list-item-title>
                    </v-list-item-content>
                  </v-list-item>
                  ...
                  <v-list-item>
                    <v-list-item-content>
                      <v-list-item-title>
                        ...
                        {{ formatTime(book_details.book_time) }} |
                        {{ book_details.star_date }}
                      </v-list-item-title>
                    </v-list-item-content>
                  </v-list-item>
                  ...
                  <v-list-item>
                    <v-list-item-content>
                      <v-list-item-title>
                        <v-chip
                          :color="
                            book_details.is_examine == 1 ? 'primary' : 'grey'
                          "
                          small
                          text-color="white"
                        >
        ...
        <v-card-actions>
          <v-btn color="info" dark @click="viewPatientDetails">
            ...
            >{{ $t("view_patient_details") || "عرض تفاصيل المريض" }}
          </v-btn>
              ...
              <v-col cols="12" sm="6" xs="12">
                ...
                <v-card outlined class="mt-3 pa-3" color="blue lighten-5">
                  ...
                  >
                    ...
                    <v-radio label="فحص" value="examination" color="primary">
                      <template v-slot:label>
                    ...
                    <v-radio label="أخرى" value="other" color="primary">
                      <template v-slot:label>
                        <div class="d-flex align-center">
                          <v-icon color="primary" small class="ml-1"
                            >mdi-text-box</v-icon
                  ...
                  <v-expand-transition>
                    <v-textarea
                      ...
                      v-model="editedItem.appointment_notes"
                      label="الملاحظات"
...
export default {
  ...
  data() {
    return {
      ...
      ArtTypeRules: [(v) => !!v || "يجب ادخال نوع المقاله"],
      selectedEvent: {},
      tokx: "",
      confirmDialog: false,
      doctors: [],
      ...
      book_details: {},
      send_msg: false,
      focus: "2026-03-01", // Focus on October by default
      startDate: "2026-03-01", // Start of October
      endDate: "2026-04-01", // End of November
      ...
      rules: {
        ...
        phoneNumber: (value) =>
          /^\d+$/.test(value) || this.$t("enter_valid_phone_number"),
      },
    };
  },
  mounted() {
    ...
    console.log(
      `🗓️ Calendar will automatically show ${currentYear}-${currentMonth
        .toString()
        .padStart(2, "0")} (current month)`,
    );

    // Initialize cache check
    console.log(
      "🚀 RequestBooking component mounted - initializing cache system",
    ); // Force calendar to show current month (October)
    this.$nextTick(() => {
  ...
  methods: {
    ...
    asyncFind(query) {
      if (query.length > 2) {
        ...
        axios
          .get(
            "https://mina-api.tctate.com/api/patients/searchv2/" + query,
            {
              headers: {
          ...
          .catch((error) => {
            ...
            const fallbackCache = localStorage.getItem(
              "clinic_app_cache_" + searchCacheKey,
            );
            if (fallbackCache) {
              try {
    ...
    async getclinicDoctor() {
      if (
        ...
        this.$store.state.role == "accounter" ||
        this.$store.state.role == "adminDoctor"
      ) {
        ...
        await axios
          .get("https://mina-api.tctate.com/api/doctors/secretary", {
            headers: {
              "Content-Type": "application/json",
              Accept: "application/json",
              Authorization: "Bearer " + this.$store.state.AdminInfo.token,
          ...
          .catch((error) => {
            ...
            // Try to use expired cache as fallback
            const fallbackCache = localStorage.getItem(
              "clinic_app_cache_" + this.cacheConfig.doctors.key,
            );
            if (fallbackCache) {
              try {
                const cacheData = JSON.parse(fallbackCache);
    ...
    async confirmDelete() {
      ...
      try {
        ...
        await axios.delete(
          `https://mina-api.tctate.com/api/reservations/${this.book_details.id}`,
          {
            headers: {
              "Content-Type": "application/json",
              Accept: "application/json",
    ...
    openReservationDialog(event) {
      ...
      this.book_details.owner_name =
        event.event.owner_name || event.event.doctor_name || "غير محدد"; // Use doctor_name as fallback
      this.book_details.doctor_name =
        event.event.doctor_name || event.event.owner_name || "غير محدد"; // Add doctor_name field
    ...
    handleSendMsg(newValue) {
      if (
        newValue &&
        this.editedItem.reservation_from_time &&
        this.editedItem.reservation_start_date
      ) {
        // Auto-generate the appointment message when switch is turned on
        const toTime =
          this.owner_item.possib_reserving_period == null
            ? this.editedItem.reservation_to_time
            : this.addtime(this.owner_item.possib_reserving_period);

        this.editedItem.appointmentMessage = `يسرنا إبلاغكم بموعدكم القادم في عيادة ${
          this.clinicName
        } بتاريخ ${
          this.editedItem.reservation_start_date
        } في الساعة ${this.convertToArabicAmPm(
          this.editedItem.reservation_from_time,
        )}${
          toTime ? " حتى " + this.convertToArabicAmPm(toTime) : ""
        }. نتمنى لكم دوام الصحة والعافية ونتطلع لرؤيتكم قريبًا.`;
      } else {
    ...
    getOwnerTctateitemsById() {
      ...
      this.$http({
        ...
        .catch((error) => {
          ...
          // Try to use expired cache as fallback
          const fallbackCache = localStorage.getItem(
            "clinic_app_cache_" + this.cacheConfig.ownerItems.key,
          );
          if (fallbackCache) {
            try {
              const cacheData = JSON.parse(fallbackCache);
    ...
    getPatient() {
      ...
      axios
        .get("https://mina-api.tctate.com/api/patients/getByUserIdv3", {
          headers: {
            "Content-Type": "application/json",
            Accept: "application/json",
            Authorization: "Bearer " + this.$store.state.AdminInfo.token,
        ...
        .catch((error) => {
          ...
          // Try to use expired cache as fallback
          const fallbackCache = localStorage.getItem(
            "clinic_app_cache_" + this.cacheConfig.patients.key,
          );
          if (fallbackCache) {
            try {
              const cacheData = JSON.parse(fallbackCache);
      ...
      if (this.$refs.form.validate()) {
        ...
        try {
          ...
          const reservationData = {
            ...
            deliverable: false,
            is_examine:
              this.editedItem.appointment_type === "examination" ? 1 : 0,
            notes:
              this.editedItem.appointment_type === "other"
                ? this.editedItem.appointment_notes
                : null,
            doctor_id: this.editedItem.doctors
              ? this.editedItem.doctors.id
              : null, // Include doctor ID when selected
            user: {
              phone:
                this.patient && this.patient.phone
                  ? "964" + parseInt(this.patient.phone.replace(/ /g, ""))
                  : "9647700281899",
          ...
          if (
            ...
            try {
              ...
              const whatsappResponse = await axios.post(
                "https://mina-api.tctate.com/api/whatsapp",
                whatsappData,
                {
                  headers: {
                    "Content-Type": "application/json",
          ...
          const doctorName = this.editedItem.doctors
            ? this.editedItem.doctors.name
            : "";
          const displayName =
            (this.$store.state.role === "secretary" ||
              this.$store.state.role === "adminDoctor" ||
              this.$store.state.role === "accountant") &&
            doctorName
          ...
          this.$swal.fire({
            ...
            icon: "success",
            title:
              this.send_msg && this.editedItem.appointmentMessage
                ? "تم الحجز وإرسال الرسالة بنجاح"
                : "تم الحجز بنجاح",
            showConfirmButton: false,
            timer: 2000,
          });
    ...
    async fetchReservations() {
      try {
        ...
        let apiEndpoint =
          "https://mina-api.tctate.com/api/reservations/formatted";
        if (
        ...
        if (response.data && response.data.status && response.data.data) {
          ...
          const formattedReservations = response.data.data
            .map((reservation) => {
              ...
              if (
                ...
                console.warn(
                  "⚠️ Skipping reservation with missing date/time:",
                  reservation,
                );
              ...
              const eventData = {
                ...
                details: `Appointment with ${patientName}`,
                phone:
                  reservation.user?.user_phone || reservation.user?.phone || "",
        ...
        // Try to use expired cache as fallback
        const fallbackCache = localStorage.getItem(
          "clinic_app_cache_" + this.currentReservationsCacheKey,
        );
        if (fallbackCache) {
          try {
            const cacheData = JSON.parse(fallbackCache);
    ...
    searchPatients(query) {
      ...
      axios
        .get(
          `https://mina-api.tctate.com/api/patients/search?query=${query}`,
          {
            headers: {
              "Content-Type": "application/json",
              Accept: "application/json",
