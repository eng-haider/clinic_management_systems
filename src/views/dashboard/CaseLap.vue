    ...
    <v-container id="case-lap" fluid tag="section">
      ...
      >
        ...
        <template v-slot:item.doctor="{ item }">
          <div v-if="item.doctor">
            ...
            <div
              ...
              v-if="item.doctor.specialization"
            >
        ...
        <template v-slot:item.actions="{ item }">
          <v-icon small class="mr-2" @click="viewCaseDetails(item)">
            mdi-eye
          </v-icon>
          <v-icon small @click="editCase(item)"> mdi-pencil </v-icon>
        </template>
        ...
        <template v-slot:no-data>
          <div class="text-center pa-4">
            ...
            <div class="text-body-2 grey--text">
              لم يتم العثور على أي حالات مختبر
            </div>
          </div>
        </template>
      </v-data-table>
    ...
    <v-dialog v-model="detailsDialog" max-width="800px">
      <v-card v-if="selectedCase">
          ...
          <v-row>
            <v-col cols="12" md="6">
              <v-card outlined>
                ...
                <v-card-text v-if="selectedCase.Patient">
                  <div>
                    <strong>الاسم:</strong> {{ selectedCase.Patient.name }}
                  </div>
                  <div>
                    <strong>الهاتف:</strong> {{ selectedCase.Patient.phone }}
                  </div>
                  <div>
                    <strong>العمر:</strong> {{ selectedCase.Patient.age }} سنة
                  </div>
                  <div
                    v-if="
                      selectedCase.patient_id && selectedCase.patient_id.address
                    "
                  >
                    <strong>العنوان:</strong>
                    {{ selectedCase.patient_id.address }}
                  </div>
                  <div
                    v-if="
                      selectedCase.patient_id &&
                      selectedCase.patient_id.systemic_conditions
                    "
                  >
                    <strong>الحالات المرضية:</strong>
                    {{ selectedCase.patient_id.systemic_conditions }}
                  </div>
                </v-card-text>
                <v-card-text v-else>
            ...
            <v-col cols="12" md="6">
              <v-card outlined>
                ...
                <v-card-text v-if="selectedCase.doctor">
                  <div>
                    <strong>الاسم:</strong> {{ selectedCase.doctor.name }}
                  </div>
                  <div v-if="selectedCase.doctor.specialization">
                    <strong>التخصص:</strong>
                    {{ selectedCase.doctor.specialization }}
                  </div>
                </v-card-text>
              </v-card>
            </v-col>
            ...
            <v-col cols="12">
              <v-card outlined>
                ...
                <v-card-text>
                  <div>
                    <strong>رقم الحالة:</strong> {{ selectedCase.case_id }}
                  </div>
                  <div>
                  ...
                  <div>
                    <strong>السعر:</strong>
                    {{ formatPrice(selectedCase.price) }} دينار
                  </div>
                  <div>
                  ...
                  <div
                    v-if="selectedCase.updated_at !== selectedCase.created_at"
                  >
        ...
        <v-card-actions>
          ...
          <v-btn color="primary" @click="editCase(selectedCase)"> تعديل </v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
...
export default {
  ...
  methods: {
    async fetchLapCases() {
      ...
      try {
        ...
        const response = await this.$http.get(
          "https://mina-api.tctate.com/api/lap-cases",
        );

        if (response.data && response.data.success) {
          this.lapCases = response.data.data || [];
          this.totalItems =
            response.data.pagination?.total || this.lapCases.length;
          ...
          this.applyFilters();
          console.log(
            `Loaded ${this.lapCases.length} lab cases (direct array)`,
          );
        } else {
          this.$swal.fire({
            icon: "warning",
    ...
    applyFilters() {
      ...
      if (this.search.status) {
        filtered = filtered.filter(
          (item) => item.status === this.search.status,
        );
      }
      ...
      if (this.search.patientName) {
        filtered = filtered.filter(
          (item) =>
            ...
            item.Patient.name
              .toLowerCase()
              .includes(this.search.patientName.toLowerCase()),
        );
      }
      ...
      if (this.search.doctorName) {
        filtered = filtered.filter(
          (item) =>
            ...
            item.doctor.name
              .toLowerCase()
              .includes(this.search.doctorName.toLowerCase()),
        );
      }
    ...
    async updateLapCaseStatus() {
      ...
      try {
        ...
        if (response.data && response.data.success) {
          // Update the local data
          const updatedCase = response.data.data;
          const index = this.lapCases.findIndex(
            (c) => c.id === this.editingCase.id,
          );
    ...
    formatDateTime(dateString) {
      ...
      return `${date.toLocaleDateString("ar-IQ")} ${date.toLocaleTimeString(
        "ar-IQ",
        {
          hour: "2-digit",
          minute: "2-digit",
